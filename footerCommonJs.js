...
{
	goStatUrl("http://tour.11st.co.kr/html/vertical/tourMain.html",code, target);
}
...
function goTotalSearch(keyword, code, popupNm){
	...
	if(popupNm != undefined){
		var url = "http://search.11st.co.kr/SearchPrdAction.tmall?method=getTotalSearchSeller&kwd=" + encodeKwd(keyword);
		goStatPopUp(url, code, popupNm);
	}else{
		var gnbFormObj = document.forms["GNBSearchForm"];
...
var htmlToastBanner = FooterComm.htmlToastBanner = {
	...
	createWrapper: function() {
		...
		var $divCont =  jQuery("<div />", {className: "toast_inner", id: "toastContWrap"});

		var $divTodayWrap = jQuery("<div />", {className: "close_wrap"});
	...
	injectionImgCont: function(){
		...
		var that = this;
		var $span = jQuery("<span />", {className: "imgonly"})
		var $a = jQuery("<a />", {href: "#"});
		var imgObj = new Image();
	...
	injectionTEXTCont: function(msg) {
		...
		var $span = jQuery("<span />", {className: "txtonly"})
		$span.append(msg);
...
function benefitCardLayer() {
	...
	if(isExceptXsite == true){
		...
		if(thisPathName == mainPath || thisPathName == mainOldPath || thisPathName == mainGenPath || thisPathName == prdDetailPath || thisPathName == mallPlanDetailPath){
							
			var layerPopStr = '<div class="dayclose_popwrap">'
					+ '		<div class="daypop">'
					+ '			<div class="daypop_cnt" id="capop_cont">'
					+ '				<h4><div class="title">내맘대로 할인 중복혜택 제외 안내</div></h4>'
					+ '				<div class="cont">'
					...
					+ '						<em>적용불가 혜택 : 내맘대로 할인/T멤버쉽/마일리지<br>카드사 : 신한/롯데/국민/현대/삼성/하나(구 하나SK)/씨티</em>'
					+ '					</div>'
					+ '				</div>'
			...
			if(thisPathName == mainGenPath || thisPathName == mainPath || thisPathName == mainOldPath) {
