...
try {
	if ( nPartnerCookieValue != '' && window._VIEW_PARTNER_HEADER != false ) {
		if(  nPartnerCookieValue == '1009' || nPartnerCookieValue == '1014' || nPartnerCookieValue == '1037'
		 ...
		 partnerHeaderHeight = 61;
		  ...
		  partnerHeaderHeight = 75;
			...
			partnerHeaderInfo = '<script type="text/javascript" src="http://static12.samsungcard.com/js/mallinmall/cml/partner/fo/cooper_e_11st.js"></script>';
			partnerHeaderHeight = 81;
			...
			partnerHeaderInfo = '<script type="text/javascript" src="http://life.kbcard.com/cxl/common/js/shop_second_gnb.js"></script>';
		...
		}else if(  nPartnerCookieValue == '1707' ){	//하나로드림
			if(document.location.protocol === "https:"){
				partnerHeaderInfo = '<script type="text/javascript" src="https://shop.dreamx.com/html/cpGNB/redir_ssl.js"></script>';
				partnerHeaderHeight = 64;
			}else{
				partnerHeaderInfo = '<script type="text/javascript" src="http://shop.dreamx.com/html/cpGNB/redir.js"></script>';
				partnerHeaderHeight = 64;
 ...
 */
function goSearch( code, gaParam ) {
	...
	if(ad_banner_url == "") {
		...
		if ( keyword == "" ) {
			...
			if ( !IsNumeric(keyword) ) {
				...
				var prdNoUrl = _GNB_CONTEXT_PATH_ + "/product/SellerProductDetail.tmall?method=getSellerProductDetail&xfrom=search^prd&prdNo=" + keyword;
				// Start 2010.11.24 검색 조건 별 선택 횟수 집계
				prdNoUrl += "&gnbTag=NO";
				// End   2010.11.24 검색 조건 별 선택 횟수 집계
...
HeaderComm.CommonLog = {
	doClick : function(inflowClfCd, trcNo, prdNo, areaClfCd, areaSubClfCd, listSortSeq, pageSize, pageNum, listSortClfCd){
		...
		if(areaSubClfCd=="01" || areaSubClfCd=="02" || areaSubClfCd=="05")
			listSortClfCd = (listSortClfCd==undefined) ? jQuery("input[name='sortCd']").val() : listSortClfCd;
		...
		jQuery.ajax({
			...
			data: {
				...
				, "trcNo"			: trcNo			// TRC_NO
				, "dispObjNo"		: prdNo			// 전시대상번호(상품번호)
				, "dispObjClfCd"	: "01"			// 전시대상구분코드 : 01(상품)
